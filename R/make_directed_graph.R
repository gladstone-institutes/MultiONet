...
#'
#' @export
make_directed_graph <- function (network, node_size = 5, label_genes = TRUE, label_size = 2) 
{
  require(OmnipathR)
  require(igraph)
  require(GGally)
  ...
  if (nrow(directed_backward) > 0) {
    for (i in seq_len(nrow(directed_backward))) {
      edge_id <- get.edge.ids(network, c(directed_backward$from[i], 
                                         directed_backward$to[i]))
      network <- delete.edges(network, edge_id)
      network <- add.edges(network, c(directed_backward$to[i], 
                                      directed_backward$from[i]))
    }
  }
  final_edges <- rbind(directed_forward, directed_backward[, 
                                                           c("to", "from")])
  
  if (nrow(final_edges) == 0) {
    message("There are no directional edges in the network based on Omnipath.")
    return(NULL)
  }
  
  directed_network <- graph_from_data_frame(final_edges, directed = TRUE)
  if (dim(get.edgelist(directed_network))[1] > 0) {
    directed_network_simple <- igraph::simplify(directed_network, 
                                                remove.multiple = TRUE, remove.loops = TRUE)
    GGally::ggnet2(directed_network_simple, label = label_genes, 
                   size = node_size, label.size = label_size, color = "steelblue", 
                   arrow.size = 10, arrow.gap = 0.025)
  }
  else {
